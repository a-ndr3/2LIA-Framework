...
public class Main {
    ...
    static KafkaEventConfig config = new KafkaEventConfig(KafkaEventConfig.ConfigType.Simple);
    ...
    public LSSKafkaProducer kafkaEventProducer() {
        return new KafkaSimpleEventProducer(config.kafkaTopic(), config.kafkaBootstrapServers());
    }

    @Bean
    public LSSKafkaListener kafkaEventListener() {
        LSSEsperQueries queries = new SimpleEsperQueries();
        return new KafkaSimpleEventListener(config.kafkaTopic(), config.kafkaBootstrapServers(), esperService, queries);
    }
}